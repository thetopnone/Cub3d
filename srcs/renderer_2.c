...
void	update_player_pos(t_game_data *game)
{
	...
	{
		player->speed = player->move_dir.y * player->move_speed;
		new_fb.x =  player->speed * player->dir.x;
		new_fb.y = player->speed * player->dir.y;
	}
	if (player->move_dir.x != 0)
	{
		player->speed = player->move_dir.x * player->move_speed;
		new_side.x = - player->speed * player->dir.y;
		new_side.y = player->speed * player->dir.x;
	}
	//printf("player pos FPS: %ld\n", get_fps(game->o_time));
	if (game->map.array[(int)(player->pos.y + new_fb.y + new_side.y + cushion.y)]
